    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        >
          ...
          <span>{t.legacyUi.backCustomerDirectory}</span>
        </button>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            ...
            <span>{t.legacyUi.manageWalletBalance}</span>
          </button>
          ...
          >
            ...
            <span>{t.legacyUi.sendDirectMessage}</span>
          </button>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 relative overflow-hidden space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-slate-800/80 pb-6">
          ...
          <div className="flex items-center gap-3 bg-slate-950 p-3.5 rounded-2xl border border-slate-800">
            <div className="px-4 text-center border-r border-slate-800">
              <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest block">{t.legacyUi.walletBalance}</span>
              <span className="text-lg font-black text-amber-400">${(wallet?.balance || 0).toFixed(2)}</span>
            </div>

            <div className="px-4 text-center">
              <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-widest block">{t.legacyUi.loyaltyPoints}</span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.totalSpending}</span>
            <span className="text-base font-black text-emerald-400">${(customer.totalSpending || customer.totalSpent || 0).toFixed(2)}</span>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.totalOrders}</span>
            <span className="text-base font-black text-white">{customer.totalOrders || 0}</span>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.avgOrderValue}</span>
            <span className="text-base font-black text-blue-400">${(customer.averageOrderValue || 0).toFixed(2)}</span>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.orderFrequency}</span>
            <span className="text-base font-black text-slate-200">{customer.orderFrequencyDays != null ? `Every ${customer.orderFrequencyDays} Days` : '—'}</span>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.cancelledOrders}</span>
            <span className="text-base font-black text-rose-400">{customer.cancelledOrders || 0}</span>
          </div>

          <div className="bg-slate-950/60 p-3.5 rounded-2xl border border-slate-800">
            <span className="text-[10px] font-bold text-slate-500 uppercase block mb-1">{t.legacyUi.refundHistory}</span>
        ...
        <div className="space-y-6">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              <span>{t.legacyUi.demographicsContact}</span>
            </h3>

            <div className="space-y-3 text-xs">
              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                <span className="text-slate-500">{translateRawUi('Gender:')}</span>
                <span className="text-slate-200 capitalize font-medium">{customer.gender || 'Unspecified'}</span>
              </div>

              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                <span className="text-slate-500">{t.legacyUi.dateOfBirthColon}</span>
                <span className="text-slate-200 font-medium">{customer.dateOfBirth || 'Not provided'}</span>
              </div>

              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                <span className="text-slate-500">{t.legacyUi.preferredLanguageColon}</span>
                <span className="text-slate-200 font-medium uppercase">{customer.preferredLanguage || 'SO'}</span>
              </div>

              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                <span className="text-slate-500">{t.legacyUi.registrationDate}</span>
              ...
              <div className="flex justify-between border-b border-slate-800/60 pb-2">
                <span className="text-slate-500">{t.legacyUi.lastOrderDate}</span>
                <span className="text-slate-200 font-medium">
              ...
              <div>
                <span className="text-slate-500 block mb-1">{t.legacyUi.deliveryAddressColon}</span>
              ...
              {customer.notes && (
                <div>
                  <span className="text-slate-500 block mb-1">{t.legacyUi.preferencesNotes}</span>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-3">
            <h3 className="text-sm font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              <span>{t.legacyUi.favoriteProducts}</span>
            </h3>
            ...
            {customer.favoriteProducts && customer.favoriteProducts.length > 0 ? (
              ...
              <p className="text-xs text-slate-500 italic">{t.legacyUi.noFavoriteDishes}</p>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-4">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-white flex items-center gap-2">
                ...
                <span>{t.legacyUi.walletTransactionLedger}</span>
              </h3>
              <button
                onClick={() => onOpenWalletModal(customer.id)}
            ...
            {walletTxs.length === 0 ? (
              <p className="text-xs text-slate-500 py-4 text-center">{t.legacyUi.noWalletTransactions}</p>
            ) : (
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 space-y-4">
            <h3 className="text-sm font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
              ...
              <span>{t.legacyUi.orderHistory20}</span>
            </h3>
            ...
            <div className="p-8 text-center text-slate-500 bg-slate-950 rounded-2xl border border-slate-800 space-y-2">
              ...
              <p className="text-[11px] text-slate-500">{t.legacyUi.allOrdersRealtime}</p>
            </div>
          </div>
        </div>
      </div>
