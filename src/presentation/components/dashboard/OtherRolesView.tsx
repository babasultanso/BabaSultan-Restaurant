  ...
  return (
    <div className="space-y-6">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="bg-purple-500/10 text-purple-400 font-bold text-[10px] px-2.5 py-0.5 rounded-full border border-purple-500/20 uppercase tracking-wider">
              {d.systemAdmin || translateRawUi('System Admin')}
            </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <KPICard title={t.legacyUi.totalOrders} value={orders.length} icon={ShieldCheck} iconColor="purple" />
        <KPICard title={translateRawUi('Enterprise Staff')} value={employees.length} icon={Users} iconColor="teal" />
        <KPICard title={translateRawUi('Security Rules')} value="v2 ABAC Active" icon={Building2} iconColor="emerald" />
        <KPICard title={translateRawUi('Database Status')} value="Firestore Synced" icon={Database} iconColor="indigo" />
      </div>
    </div>
  );
};
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <span className="bg-emerald-500/10 text-emerald-400 font-bold text-[10px] px-2.5 py-0.5 rounded-full border border-emerald-500/20 uppercase tracking-wider block w-fit mb-1">
            {d.floorStaff || translateRawUi('Floor Staff')}
          </span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard title={translateRawUi('Ready for Table Service')} value={readyTableOrders.length} icon={Utensils} iconColor="emerald" />
        <KPICard title={translateRawUi('Active Occupied Tables')} value={activeDineIn.length} icon={Clock} iconColor="amber" />
        <KPICard title={translateRawUi('Floor Shift Sales')} value={`$${orders.reduce((s, o) => s + o.totalAmount, 0).toFixed(2)}`} icon={CheckCircle2} iconColor="teal" />
      </div>

      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <h3 className="text-sm font-bold text-white flex items-center gap-2 border-b border-slate-800 pb-3">
          <Utensils className="w-5 h-5 text-emerald-400" />
          {translateRawUi('Kitchen Ready Dishes awaiting Table Delivery')}
        </h3>
        <div className="space-y-2">
          {readyTableOrders.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-6">{translateRawUi('No kitchen dishes currently waiting for floor delivery.')}</p>
          ) : (
            readyTableOrders.map(o => (
              <div key={o.id} className="flex items-center justify-between bg-slate-950 p-3.5 rounded-2xl border border-emerald-500/30 text-xs">
                <div>
                  <span className="font-bold text-white">{o.orderNumber} ({o.tableNumber || '—'})</span>
                  <p className="text-[10px] text-slate-400">{o.items?.map(i => `${i.quantity}x ${i.productName}`).join(', ')}</p>
                </div>
                <span className="text-xs font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20">{translateRawUi('READY')}</span>
              </div>
            ))
          )}
        </div>
...
export const DeliveryDriverView: React.FC<OtherRolesViewProps> = ({ orders }) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 p-6 rounded-3xl border border-slate-800 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <span className="bg-indigo-500/10 text-indigo-400 font-bold text-[10px] px-2.5 py-0.5 rounded-full border border-indigo-500/20 uppercase tracking-wider block w-fit mb-1">
            {translateRawUi('Dispatch Portal')}
          </span>
          <h2 className="text-2xl font-bold text-white">{t.legacyUi.deliveryDriverApp}</h2>
          <p className="text-xs text-slate-400">{t.legacyUi.assignedDeliveryHelp}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <KPICard title={translateRawUi('Active Assigned Deliveries')} value={activeDeliveries.length} icon={Truck} iconColor="indigo" />
        <KPICard title={translateRawUi('Unread Dispatch Alerts')} value={driverNotifs.filter(n => !n.read).length} icon={Bell} iconColor="amber" />
        <KPICard title={translateRawUi('Completed Deliveries')} value={completedDeliveries.length} icon={CheckCircle2} iconColor="emerald" />
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <h3 className="text-sm font-bold text-white flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span>{t.legacyUi.driverRealtimeDispatch}</span>
          </div>
        ...
        <div className="space-y-2">
          {driverNotifs.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-4">{translateRawUi('No dispatch notifications received yet.')}</p>
          ) : (
            driverNotifs.map(n => (
              <div
                key={n.id}
              ...
              >
                ...
                {!n.read && (
                  ...
                  >
                    <Check className="w-3 h-3" /> {translateRawUi('Mark Read')}
                  </button>
                )}
              </div>
            ))
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-4">
        <h3 className="text-sm font-bold text-white flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2">
            ...
            <span>{translateRawUi('My Assigned Deliveries')}</span>
          </div>
        ...
        <div className="space-y-4">
          {assignedDeliveries.length === 0 ? (
            <div className="text-center py-12 bg-slate-950/40 rounded-2xl border border-slate-800/60">
              ...
              <p className="text-sm font-bold text-white mb-1">{translateRawUi('No Deliveries Assigned')}</p>
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                {translateRawUi('When the dispatcher assigns delivery orders to your account, they will appear here in real-time.')}
              </p>
            </div>
          ) : (
                ...
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400 mt-0.5 font-medium">{translateRawUi('Customer:')} <strong className="text-white">{d.customerName}</strong></p>
                      </div>
                    </div>
                  ...
                  {isDelivered && (
                    <div className="flex items-center gap-2 text-emerald-400 text-xs font-bold pt-1">
                      <CheckCircle2 className="w-4 h-4" />
                      <span>{translateRawUi('Trip successfully completed and delivered')}</span>
                    </div>
                  )}
                </div>
              );
